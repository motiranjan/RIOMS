...
namespace RIOMS.Domain
{
    ...
    public partial class CollectionCess
    {
        ...
        {
            MoreThanThree = iFormDetailCess.Sum(c => c.MoreThanThree).GetValueOrDefault();
            Third = iFormDetailCess.Sum(c => c.Third).GetValueOrDefault();
            Second = iFormDetailCess.Sum(c => c.Second).GetValueOrDefault();
            Previous = iFormDetailCess.Sum(c => c.Previous).GetValueOrDefault();
            Current = iFormDetailCess.Sum(c => c.Current).GetValueOrDefault();
        }
    ...
    public class BalanceCess:PartOneRev
    {
        public BalanceCess()
        {
        ...
        public BalanceCess(CollectionCess collection, DemandCess demandCess)
        {
            ...
            if ((demandCess.Advance + collection.Current + Advance) > demandCess.Current)
            {
            ...
            {
                Current = demandCess.Current - (collection.Current + Advance + demandCess.Advance);
                Advance = 0;
            }
    ...
    public class BalanceWaterTax:PartOneRev
{
    ...
    public class BalanceLandRevenue:PartOneRev
    {
